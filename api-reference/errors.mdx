...
description: 'Learn through the errors and rate-limits that could happen whilst using the API'
---
...
<Warning>
  `4xx` will be inside the response JSON (`response.status`), whilst `5xx` errors are sent as an HTTP status code.
...
All errors will include an `response.error` attribute detailing the error message.
...
Our rate-limiting system is based on 10-seconds intervals rather than 1, meaning that you can do a burst of API call within the first second, then wait 9 seconds and do it again.
...
<Note>To upgrade to an higher tier, please reach out to us at sales@sellix.io or through our [Discord community](https://sellix.io/discord)</Note>
...
If you are on the standard version, payment endpoints are highly restricted compared to standard ones.
