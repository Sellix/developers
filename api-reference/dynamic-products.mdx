...
If you do not want to specify the product's value with the serials type, you can setup your own webhook and dynamically return to us the product you want to ship to your customers.
...
The webhook event we will send is `product:dynamic`, which will have as payload the [order object](/api-reference/orders/get-order).
...
<Info>This is particularly useful if the invoice type is a `SHOPPING_CART`, containing multiple dynamic products. In this case we will send more than one `product:dynamic` events, one for each product present in the shopping cart.</Info>
