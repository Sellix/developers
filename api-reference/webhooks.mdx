...
A webhook simulator is available allowing you to simulate webhooks to a URL. It can be accessed [here](https://dashboard.sellix.io/developer/webhooks/logs).
...
| subscription:cancelled | A subscription has been cancelled, either due to not receiving a payment or by a manual action of the customer. |
| subscription:upcoming | A customer will have to pay one of your subscriptions in the next 72 hours. |
| order:paid:product | This event is the same as order:paid, however :products indicates that it originates from a webhook URL configured within a product, which is now deprecated |
...
Each webhook request will create a [Webhook Log](https://dashboard.sellix.io/developer/webhooks/all). The object is created by the request has been sent. Before the request response has actually been received, the `response_code` will be 0, indicating it is pending.
