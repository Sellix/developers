...
| <Tooltip tip="Subscriptions 2.0 bring 40+ new webhook events that enable developers to fully customize and automate their payment flow. Notably, we now have separate events for when the subscription expires and when it is cancelled, separating the user action versus the actual cancellation and expiration of the subscription. These new events also include upgrades, downgrades, changes, failed payments and more.">Full Webhook events</Tooltip> | Live | - |
...
| <Tooltip tip="Through the Sellix SDK and API merchant can embed sections of the customer portal inside their websites.">Embeddables</Tooltip> | Coming Soon | Q3 2025 |
...
<Warning>
  This journey is a work in progress, it's not final. Expected to be completed soon, check back in a few days.
</Warning>

<Tabs>
  <Tab title="Basic Jounrey">
    <Steps>
      ...
      <Step title="Subscription Canceled">
        Event `subscription-v2:canceled`
        ...
        - `subscription-v2:canceled-merchant`: When the subscription is canceled by the merchant through their Sellix dashboard.
        - `subscription-v2:canceled-customer`: When the subscription is canceled by the customer through an embeddable or the customer dashboard.
        - `subscription-v2:canceled-api`: When the subscription is canceled by calling the `DELETE /subscriptions_v2/{uniqid}` endpoint.
      </Step>
      ...
      <Step title="Subscription Failed">
        ...
        The payment for the subscription has failed. The subscription will remain `ACTIVE` and the payment will be retried at a later time. A CRON job will cancel subscriptions with invoice's that were `VOID`ed
        If the payment is not updated, the subscription will be canceled and the `subscription-v2:canceled` event will be triggered. 
      </Step>
    </Steps>
  </Tab>
  <Tab title="with Trial">
    <Steps>
      ...
      <Step title="Trial Canceled">
        ...
        *Only sent if a trial period is configured for this subscription product*
        ...
        - `subscription-v2:trial-interval:canceled-customer`: When the subscription is canceled by the customer through an embeddable or the customer dashboard.
      ...
      <Step title="Trial Ended">
        ...
        *Only sent if a trial period is configured for this subscription product*
        ...
        Immediately after this event is triggered, the `subscription-v2:payment-interval:started` will be sent indicating that the first payment interval has been requested. **This does not mean that the payment was received**.
      </Step>
      <Step title="Subscription Started">
        Event `subscription-v2:started`
  ...
  <Tab title="Advanced Journey">
    <Steps>
      ...
      <Step title="Subscription Gateway Changes">
        ...
        If the customer updates his preferred gateway for the subscription through embeddables or the Customer Billing Portal, you will receive a webhook event with the updated subscription object.
      ...
      <Step title="Subscription Updates">
        Event `subscription-v2:plan:upgrade` or `subscription-v2:plan:downgrade`

        When the customer or merchant changes the plan of their subscription, one of these events will be triggered. 
      ...
      <Step title="Subscription Renewal Approaching">
        Event `subscription-v2:payment-interval:upcoming`
